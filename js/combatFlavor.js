...
const POOLS = {
  ...
  en: {
    ...
    divineSmite: [
      `${iconImg('✨','14px')} Holy radiance erupts as {n}'s weapon strikes {t}!`,
      `${iconImg('✨','14px')} {n} channels divine power through their blade — radiant justice!`,
      `${iconImg('✨','14px')} The gods themselves guide {n}'s smiting blow against {t}!`,
      `${iconImg('✨','14px')} Light blazes from {n}'s weapon as Divine Smite obliterates {t}'s defense!`,
    ],
    divineSmiteUndead: [
      `${iconImg('✨','14px')} Sacred flame scorches {t} — undead abominations burn twice as bright!`,
      `${iconImg('✨','14px')} {n}'s smite detonates against the unholy form of {t}. DOUBLE damage!`,
      `${iconImg('✨','14px')} Holy wrath, amplified. {n} tears through {t}'s undead essence!`,
    ],
    wildShape: [
      `${iconImg('🐾','14px')} In a blur of shifting flesh, {n} becomes a {x}!`,
      `${iconImg('🐾','14px')} {n}'s form ripples — fur and fang take over. A {x} stands in their place!`,
      `${iconImg('🐾','14px')} Wild Shape: {n} embraces the primal form of the {x}!`,
      `${iconImg('🐾','14px')} The transformation is instant. Where {n} stood, a {x} now prowls.`,
    ],
    endWildShape: [
      `${iconImg('🐾','14px')} {n} releases the beast's form, returning to themselves.`,
      `${iconImg('🐾','14px')} The wild shape dissolves — {n} stands tall once more.`,
      `${iconImg('🐾','14px')} With a shudder, the animal form fades. {n} is back.`,
    ],
    summonAnimal: [
      `${iconImg('🐺','14px')} {n} calls upon nature's bond — a {x} answers.`,
      `${iconImg('🐺','14px')} From the wilds, {n} summons a faithful {x} companion!`,
      `${iconImg('🐺','14px')} A {x} emerges from the undergrowth, answering {n}'s call.`,
    ],
    turnUndeadFailed: [
      `${iconImg('✝️','14px')} {n}'s holy symbol blazes — {t} recoils in holy terror!`,
      `${iconImg('✝️','14px')} Sacred light floods the area. {t} cannot withstand {n}'s divine power!`,
      `${iconImg('✝️','14px')} Turn Undead: the creature {t} is repelled by {n}'s faith!`,
    ],
    turnUndeadResisted: [
      `${iconImg('✝️','14px')} {t} resists {n}'s turning — dark power holds firm.`,
      `${iconImg('✝️','14px')} The undead {t} stares down {n}'s holy symbol. It holds.`,
      `${iconImg('✝️','14px')} {n}'s turning attempt fails — {t} seems fortified against divinity.`,
    ],
    bardicInspiration: [
  ...
  he: {
    ...
    divineSmite: [
      `${iconImg('✨','14px')} קרינה קדושה פורצת כשנשק {n} פוגע ב{t}!`,
      `${iconImg('✨','14px')} {n} מעביר כוח אלוהי דרך להבם — צדק ורדרד!`,
      `${iconImg('✨','14px')} האלים עצמם מנחים את מכת השמיטה של {n} נגד {t}!`,
      `${iconImg('✨','14px')} אור בוהק מנשק {n} כשDivine Smite מנפץ את הגנת {t}!`,
    ],
    divineSmiteUndead: [
      `${iconImg('✨','14px')} להבה קדושה שורפת את {t} — ישויות בלתי-חיות נשרפות כפליים!`,
      `${iconImg('✨','14px')} מכת השמיטה של {n} מתפוצצת על צורת ה{t} הבלתי-חיה. נזק כפול!`,
      `${iconImg('✨','14px')} זעם קדוש, מוגבר. {n} קורע דרך מהות ה{t} הבלתי-חיה!`,
    ],
    wildShape: [
      `${iconImg('🐾','14px')} בטשטוש בשר משתנה, {n} הופך ל{x}!`,
      `${iconImg('🐾','14px')} צורת {n} מתרסקת — פרווה וניבים משתלטים. {x} עומד במקומם!`,
      `${iconImg('🐾','14px')} Wild Shape: {n} מאמץ את הצורה הפרימורדיאלית של {x}!`,
      `${iconImg('🐾','14px')} השינוי מיידי. איפה {n} עמד, {x} מסייר כעת.`,
    ],
    endWildShape: [
      `${iconImg('🐾','14px')} {n} משחרר את צורת החיה, חוזר לעצמם.`,
      `${iconImg('🐾','14px')} הצורה הפראית מתמוססת — {n} עומד זקוף שוב.`,
      `${iconImg('🐾','14px')} עם רעד, צורת החיה דועכת. {n} חזר.`,
    ],
    summonAnimal: [
      `${iconImg('🐺','14px')} {n} קורא לקשר הטבע — {x} עונה לקריאה.`,
      `${iconImg('🐺','14px')} מהפרא, {n} מזמן חבר {x} נאמן!`,
      `${iconImg('🐺','14px')} {x} יוצא מהצמחייה, עונה לקריאת {n}.`,
    ],
    turnUndeadFailed: [
      `${iconImg('✝️','14px')} סמל הקדוש של {n} בוהק — {t} נסוג בפחד קדוש!`,
      `${iconImg('✝️','14px')} אור קדוש מציף את האזור. {t} אינו יכול לעמוד בכוח {n} האלוהי!`,
      `${iconImg('✝️','14px')} Turn Undead: הברייה {t} נדחית על ידי אמונת {n}!`,
    ],
    turnUndeadResisted: [
      `${iconImg('✝️','14px')} {t} עומד בפני פנייתו של {n} — כוח האופל מחזיק חזק.`,
      `${iconImg('✝️','14px')} ה{t} הבלתי-חי מתעמת עם הסמל הקדוש של {n}. זה מחזיק.`,
      `${iconImg('✝️','14px')} ניסיון ה{n} לגרש נכשל — {t} נראה מחוזק כנגד האלוהות.`,
    ],
    bardicInspiration: [
